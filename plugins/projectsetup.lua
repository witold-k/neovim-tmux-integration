...
function M.set_python_path(script_path)
    ...
    if not pp then
        ...
        if not string.find(pp, 'common-scripts') then
            vim.fn.setenv('PYTHONPATH', pp .. ':' .. project_dir .. '/modules/common-scripts/python')
            pp = os.getenv('PYTHONPATH') or '' -- or '' will never happen since set before
        end
        if not string.find(pp, '/python3.11/site-packages') then
            vim.fn.setenv('PYTHONPATH', pp .. ':' ..  vim.env.HOME .. '/python/lib/python3.11/site-packages')
        end
    end
end
...
function M.project_setup_bitbake_just(script_path)
    M.set_python_path(script_path)
    local project_dir = tostring(vim.fn.expand(vim.fn.fnamemodify(script_path , ":h:h")))
...
function M.project_setup_bitbake_make(script_path)
    M.set_python_path(script_path)
    local project_dir = tostring(vim.fn.expand(vim.fn.fnamemodify(script_path , ":h:h")))
